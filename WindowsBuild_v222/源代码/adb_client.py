...
def _chmod_x_if_needed(p: str) -> None:
    """确保 adb 二进制拥有可执行位（某些解压/打包环境会丢失）。Windows 下跳过。"""
    if _is_windows():
        return
    try:
...
def _find_adb() -> str:
    """
    自动查找 adb 可执行文件路径，按优先级依次搜索（跨平台兼容 macOS / Windows）：
    1. PyInstaller 打包后内置的 platform-tools/adb[.exe]
       - onefile 模式：sys._MEIPASS (临时解压目录)
       - onedir macOS .app : Contents/MacOS, Contents/Frameworks, Contents/Resources
       - onedir Windows   : exe 同级目录, exe 同级的 _internal/ 子目录
    2. 脚本同级目录下的 platform-tools/adb[.exe]（开发模式）
    3. 系统常见安装路径（Android Studio / Homebrew / Windows AppData / Scoop / Chocolatey）
    4. PATH 环境变量中的 adb[.exe]
    """
    WIN = _is_windows()
    ADB_BIN = 'adb.exe' if WIN else 'adb'

    def _check(p: str) -> Optional[str]:
        if os.path.isfile(p):
            _chmod_x_if_needed(p)
            # Windows: .exe 存在即可执行，不依赖 os.access(p, X_OK)
            if WIN:
                return p
    ...
    candidates: List[str] = []
    if frozen:
        # 1a) onefile 模式：MEIPASS 是临时解压根（平台无关）
        if meipass:
            candidates.append(os.path.join(meipass, 'platform-tools', ADB_BIN))
        # 1b) onedir 模式
        try:
            exe_path = os.path.abspath(sys.executable)
            exe_dir = os.path.dirname(exe_path)
            if WIN:
                # Windows onedir: datas 会落到 exe_dir\_internal\platform-tools\
                #                 也会落到 exe_dir\platform-tools\
                candidates.append(os.path.join(exe_dir, '_internal', 'platform-tools', ADB_BIN))
                candidates.append(os.path.join(exe_dir, 'platform-tools', ADB_BIN))
                candidates.append(os.path.join(os.path.dirname(exe_dir), 'platform-tools', ADB_BIN))
            elif '.app' in exe_path:
                # macOS .app bundle：datas 落到 Contents/Frameworks/、Contents/MacOS/ 或 Contents/Resources/
                contents = None
                ...
                if contents:
                    for sub in ('Frameworks', 'MacOS', 'Resources'):
                        candidates.append(os.path.join(contents, sub, 'platform-tools', ADB_BIN))
            else:
                # macOS/Linux onedir 兜底
                candidates.append(os.path.join(exe_dir, 'platform-tools', ADB_BIN))
                candidates.append(os.path.join(os.path.dirname(exe_dir), 'platform-tools', ADB_BIN))
        except Exception:
            pass
    ...
    # ------------------------------------------------------------
    # 2. 开发模式：脚本同级目录下的 platform-tools/adb[.exe]
    # ------------------------------------------------------------
    script_dir = os.path.dirname(os.path.abspath(__file__))
    local_adb = os.path.join(script_dir, 'platform-tools', ADB_BIN)
    if _check(local_adb):
        return local_adb
    ...
    if WIN:
        ...
        common_paths = [
            os.path.join(local_appdata, 'Android', 'Sdk', 'platform-tools', ADB_BIN),
            os.path.join(home, 'AppData', 'Local', 'Android', 'Sdk', 'platform-tools', ADB_BIN),
            os.path.join(program_files, 'Android', 'Android Studio', 'plugins', 'android-ndk', 'resources', 'platform-tools', ADB_BIN),
            os.path.join(program_files, 'Android', 'Android Studio', 'Contents', 'android-sdk', 'platform-tools', ADB_BIN),
            os.path.join(program_files_x86, 'Android', 'android-sdk', 'platform-tools', ADB_BIN),
            os.path.join(program_files, 'android-sdk', 'platform-tools', ADB_BIN),
            os.path.join(home, 'android-sdk', 'platform-tools', ADB_BIN),
            os.path.join(home, '.android-sdk', 'platform-tools', ADB_BIN),
            # Scoop / Chocolatey
            os.path.join(local_appdata, 'scoop', 'shims', ADB_BIN),
            os.path.join(program_files_data, 'chocolatey', 'bin', ADB_BIN) if (program_files_data := os.environ.get('PROGRAMDATA', r'C:\ProgramData')) else '',
            # 解压到 C 盘根目录
        ...
        # macOS / Linux 常见路径
        common_paths = [
            os.path.join(home, 'Library', 'Android', 'sdk', 'platform-tools', ADB_BIN),
            os.path.join(home, 'Android', 'Sdk', 'platform-tools', ADB_BIN),
            os.path.join(home, '.android-sdk', 'platform-tools', ADB_BIN),
    ...
    # 4. 从 PATH 中查找
    # ------------------------------------------------------------
    found = shutil.which('adb') or (shutil.which('adb.exe') if WIN else None)
    if found:
        return found

    return 'adb.exe' if WIN else 'adb'  # 回退到默认值，由调用时报错提示
        ...
        try:
            ...
            if _is_windows():
                raise RuntimeError(
                    ...
                    "请通过以下方式之一解决：\n"
                    "1. 已内置 platform-tools，若异常请手动下载 Windows 版:\n"
                    "   https://dl.google.com/android/repository/platform-tools-latest-windows.zip\n"
                    "   解压后将 platform-tools 目录放在 EXE 同级目录即可\n"
                    "2. 或使用 Scoop: scoop install android-platform-tools\n"
                    "3. 或使用 Chocolatey: choco install adb"
                )
            else:
                raise RuntimeError(
                    f"找不到ADB可执行文件 (尝试路径: {self.adb_path})。\n"
